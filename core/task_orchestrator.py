...
class TaskStore:
    ...
    @contextmanager
    def _conn(self):
        DB.parent.mkdir(parents=True,exist_ok=True); c=sqlite3.connect(DB,timeout=10)
        try:
            c.execute("PRAGMA journal_mode=WAL"); c.execute("PRAGMA busy_timeout=10000"); yield c; c.commit()
        except Exception:
            c.rollback(); raise
        finally: c.close()
    def init(self):
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            while pending or running:
                for t in list(pending.values()):
                    ...
                    if "failed" in ds or "cancelled" in ds:self.store.update(t.task_id,"cancelled",error="dependency_failed");done[t.task_id]={"task_id":t.task_id,"state":"cancelled","error":"dependency_failed"};pending.pop(t.task_id,None)
                    elif "waiting_confirmation" in ds:self.store.update(t.task_id,"waiting_confirmation",error="dependency_confirmation_required");done[t.task_id]={"task_id":t.task_id,"state":"waiting_confirmation","error":"dependency_confirmation_required"};pending.pop(t.task_id,None)
