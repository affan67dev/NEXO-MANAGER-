"""Controlled Android/Termux capability layer for ALEX.

Only explicit, registered capabilities belong here. No arbitrary shell execution is exposed.
"""
...
SCREEN_DIR = Path.home() / "NEXO" / "screen"
...
def capabilities() -> dict[str, Any]:
    return {
        "termux_api": _exists("termux-battery-status"),
        "screen_capture": _exists("termux-screenshot"),
        "toast": _exists("termux-toast"),
        "tts": _exists("termux-tts-speak"),
        "stt": _exists("termux-speech-to-text"),
        "microphone_record": _exists("termux-microphone-record"),
...
def capture_screen() -> dict[str, Any]:
    ...
    if not result["ok"] or not target.exists() or target.stat().st_size == 0:
        return {"ok": False, "verified": False, "available": True,
                "error": "screen_capture_failed", "detail": result["stderr"] or result["stdout"]}
    digest = hashlib.sha256(target.read_bytes()).hexdigest()
    record_event("screen_capture", {"timestamp": time.time(), "sha256": digest, "path": str(target)})
...
def analyze_screen() -> dict[str, Any]:
    ...
    observation: dict[str, Any] = {"screen_captured": True, "observed": [], "inferred": [], "uncertain": []}
    if _exists("tesseract"):
        result = _run(["tesseract", str(path), "stdout", "--psm", "6"], 30)
        if result["ok"] and result["stdout"]:
            observation["observed"] = [line.strip() for line in result["stdout"].splitlines() if line.strip()][:80]
        else:
            observation["uncertain"].append("OCR did not return readable text.")
    else:
...
def toast_state(state: str) -> dict[str, Any]:
    allowed = {"IDLE", "LISTENING", "PROCESSING", "SPEAKING"}
