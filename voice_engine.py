...
def record(seconds: int = RECORD_SECONDS) -> Optional[Path]:
    ...
    if not result["ok"] or not filename.exists() or filename.stat().st_size < 100: return None
    return filename
...
def is_wake_word(text: str) -> bool:
    ...
    return any(value == w or value.startswith(w + " ") for w in WAKE_WORDS)
...
def detect_wake_word_from_audio(audio_file: Path | str) -> bool:
    ...
    if model_path and importlib.util.find_spec("openwakeword") is not None:
        try:
            ...
            with wave.open(str(audio_file), "rb") as wav: raw, rate = wav.readframes(wav.getnframes()), wav.getframerate()
            if rate != 16000: return False
            scores = Model(wakeword_models=[model_path], inference_framework="onnx").predict(np.frombuffer(raw, dtype=np.int16))
...
def execute_via_nexo(text: str, *, owner: bool = True, user_id: int | str | None = None, confirmation: bool = False, planner: ExecutivePlanner | None = None) -> dict[str, Any]:
    ...
    def executor(name: str, args: dict[str, Any], **kw: Any) -> dict[str, Any]:
        result = _execute_voice_tool(name, args, owner=owner, user_id=user_id, confirmation=confirmation)
        observed.append({"name": name, "args": args, "result": result})
        return result
    ...
    requires_confirmation = any(x["result"].get("requires_confirmation") for x in observed)
    verified = bool(observed) and all(x["result"].get("verified") is True for x in observed) and not requires_confirmation
    return {"ok": bool(observed) and not requires_confirmation, "verified": verified, "response": answer, "agent": task.agent, "intent": task.intent, "tools": observed, "requires_confirmation": requires_confirmation}

class NexoVoiceAssistant:
    def __init__(self, stt: Callable[[Path | str], dict[str, Any]] = speech_to_text, tts: Callable[[str], bool] = speak, recorder: Callable[[int], Optional[Path]] = record): self.stt,self.tts,self.recorder=stt,tts,recorder
    def process_text(self, text: str, owner: bool = True, user_id: int | str | None = None, confirmation: bool = False, planner: ExecutivePlanner | None = None) -> VoiceResult:
        ...
        if outcome["requires_confirmation"]: response="I need your explicit confirmation before I do that."
    ...
    def run_forever(self, seconds: int = 3, owner: bool = True, user_id: int | str | None = None) -> None:
        while True:
            ...
            if not detect_wake_word_from_audio(audio): continue
            stt=self.stt(audio)
            if stt.get("ok"): self.process_text(stt.get("text",""),owner=owner,user_id=user_id)
