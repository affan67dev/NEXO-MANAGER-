"""NEXO voice runtime: microphone -> wake -> request analysis -> orchestration -> TTS."""
...
RECORD_SECONDS=int(os.getenv("NEXO_VOICE_RECORD_SECONDS","6")); LLAMA_URL=os.getenv("LLAMA_URL","http://127.0.0.1:8080/v1/chat/completions").strip()
...
@dataclass
class VoiceResult:
    ok:bool; text:str=""; response:str=""; stage:str=""; provider:str=""; verified:bool=False; details:dict[str,Any]=field(default_factory=dict)
    def as_dict(self)->dict[str,Any]: return self.__dict__.copy()

def _run(command:list[str],timeout:int=30)->dict[str,Any]:
    try:
        p=subprocess.run(command,capture_output=True,text=True,timeout=timeout,check=False); return {"ok":p.returncode==0,"stdout":p.stdout.strip(),"stderr":p.stderr.strip(),"code":p.returncode}
    except (OSError,subprocess.TimeoutExpired) as exc:return {"ok":False,"stdout":"","stderr":str(exc),"code":-1}
...
def speak(text:str)->bool:
    ...
    if platform.system()=="Windows" and _exists("powershell"):return _run(["powershell","-NoProfile","-Command","Add-Type -AssemblyName System.Speech; (New-Object System.Speech.Synthesis.SpeechSynthesizer).Speak($args[0])",text],30)["ok"]
...
def termux_speech_to_text()->dict[str,Any]:
    if not _exists("termux-speech-to-text"):return {"ok":False,"provider":"termux-speech-to-text","error":"Termux:API speech command unavailable"}
    result=_run(["termux-speech-to-text"],90)
    if not result["ok"] or not result["stdout"]:return {"ok":False,"provider":"termux-speech-to-text","error":result["stderr"] or "speech recognition returned no text"}
...
def _faster_whisper(audio:Path)->dict[str,Any]:
    ...
    try:
        ...
        engine=WhisperModel(model,device=os.getenv("NEXO_WHISPER_DEVICE","cpu"),compute_type=os.getenv("NEXO_WHISPER_COMPUTE","int8")); segments,info=engine.transcribe(str(audio),vad_filter=True); text=" ".join(s.text.strip() for s in segments).strip()
...
def speech_to_text(audio_file:Path|str|None=None)->dict[str,Any]:
    if audio_file is None:return termux_speech_to_text()
...
def is_wake_word(text:str)->bool:
    value=normalize(text); return any(re.match(r"^"+re.escape(w)+r"(?:\s|[,.!?;:]|$)",value) for w in WAKE_WORDS)

def remove_wake_word(text:str)->str:
    value=text or ""
    for word in WAKE_WORDS:value=re.sub(r"^"+re.escape(word)+r"(?:\s|[,.!?;:]*)","",value,flags=re.I)
    return value.strip(" ,.!?;:")

def _execute_voice_tool(name:str,args:dict[str,Any],*,owner:bool=True,user_id:int|str|None=None,confirmation:bool=False)->dict[str,Any]:
...
def _fallback_plan(text:str)->list[dict[str,Any]]:
    """Conservative fallback when LLaMA cannot return a plan; it never invents tools."""
    ...
    for part in parts[:20]:
        tid=f"task-{uuid.uuid4().hex[:10]}"; deps=[previous] if previous else []
        result.append({"task_id":tid,"objective":part,"dependencies":deps,"priority":"normal","kind":"dependent" if deps else "sequential","expected_result":f"Successful completion of: {part}","max_retries":2,"timeout_seconds":90,"parameters":{}}); previous=tid
    return result
...
def execute_via_nexo(text:str,*,owner:bool=True,user_id:int|str|None=None,confirmation:bool=False,planner:ExecutivePlanner|None=None)->dict[str,Any]:
    ...
    if not plan: plan=_fallback_plan(text)
    tasks=specs_from_plan(plan)
    if not tasks: tasks=[TaskSpec(objective=text)]
    # A dependency chain is safer than pretending multiple natural-language actions are independent.
    if len(tasks)>1:
        ids={t.task_id for t in tasks}
        for i,t in enumerate(tasks):
            t.dependencies=[d for d in t.dependencies if d in ids]
    orchestrator=TaskOrchestrator(); observed=[]
    def execute_task(task:TaskSpec)->dict[str,Any]:
        local=[]
        def executor(name,args,**kw):
            result=_execute_voice_tool(name,args,owner=owner,user_id=user_id,confirmation=confirmation); local.append({"name":name,"args":args,"result":result}); observed.append({"task_id":task.task_id,"name":name,"args":args,"result":result}); return result
        messages=[{"role":"system","content":"You are NEXO's execution planner. Understand the objective, select only registered tools, respect dependencies already completed, and never claim success unless a tool returns verified=true."},{"role":"user","content":task.objective}]
        try:
            answer=planner.run(task.objective,messages,schemas(),executor,owner=owner,user_id=user_id,max_steps=6)
        except Exception as exc:
            return {"ok":False,"verified":False,"permanent":False,"error":f"llama_execution_failed:{type(exc).__name__}","response":"I couldn't execute that task because the reasoning service failed."}
    ...
    try: results=orchestrator.run(request_id,tasks,execute_task,max_parallel=2)
    except Exception as exc: return {"ok":False,"verified":False,"response":"I couldn't safely complete the task plan.","agent":manager_task.agent,"intent":manager_task.intent,"tools":observed,"tasks":[],"error":type(exc).__name__}
    requires=any(r.get("requires_confirmation") for r in results); all_verified=bool(results) and all(r.get("state")=="completed" and r.get("verified") for r in results) and not requires
    failed=[r for r in results if r.get("state")=="failed"]
    response="I need your explicit confirmation before I do that." if requires else ("Some tasks failed, but I preserved the remaining task state." if failed else str(results[-1].get("response") or "Task completed."))
    return {"ok":all_verified,"verified":all_verified,"response":response,"agent":manager_task.agent,"intent":manager_task.intent,"tools":observed,"tasks":results,"request_id":request_id,"requires_confirmation":requires}
...
class NexoVoiceAssistant:
    ...
    def process_text(self,text:str,owner:bool=True,user_id:int|str|None=None,confirmation:bool=False,planner:ExecutivePlanner|None=None)->VoiceResult:
        ...
        if is_wake_word(text):
            ...
            if not text:
                response="Yes, how can I help you?"; spoken=self.tts(response); return VoiceResult(True,response=response,stage="wake",provider="text",verified=spoken,details={"tts_verified":spoken})
        outcome=execute_via_nexo(text,owner=owner,user_id=user_id,confirmation=confirmation,planner=planner); response=outcome["response"]; spoken=self.tts(response)
