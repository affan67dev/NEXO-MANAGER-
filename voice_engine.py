...
import importlib.util, json, os, platform, re, shutil, subprocess, time
...
def execute_via_nexo(text:str,*,owner:bool=True,user_id:int|str|None=None,confirmation:bool=False,planner:ExecutivePlanner|None=None)->dict[str,Any]:
    ...
    requires=any(x["result"].get("requires_confirmation") for x in observed); verified=bool(observed) and all(x["result"].get("verified") is True for x in observed) and not requires
...
class NexoVoiceAssistant:
    ...
    def process_text(self,text:str,owner:bool=True,user_id:int|str|None=None,confirmation:bool=False,planner:ExecutivePlanner|None=None)->VoiceResult:
        ...
        outcome=execute_via_nexo(text,owner=owner,user_id=user_id,confirmation=confirmation,planner=planner); response="I need your explicit confirmation before I do that." if outcome["requires_confirmation"] else outcome["response"]; spoken=self.tts(response)
    ...
    def run_forever(self,owner:bool=True,user_id:int|str|None=None)->None:
        while True:
            ...
            if not command:
                self.tts("Yes, how can I help you?"); continue
