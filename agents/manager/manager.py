...
class NexoManager:
    ...
    def in_scope(self, text: str) -> bool:
        t = text.lower()
        keywords = {
            "app","application","backend","api","server","database","user","support","email","whatsapp",
            "security","report","moderation","bug","error","crash","log","deployment","testing","monitoring",
            "authentication","login","performance","code","youtube","instagram","telegram","chrome","google",
            "open","launch","khol","kholo","play","song","music","wifi","torch","battery","split","window","pip",
            "device","phone","tablet","android","computer","mac","windows"
        }
    ...
    def create_task(self, request: str) -> Task:
        if not self.in_scope(request): return Task(request=request, intent="out_of_scope", agent="manager")
...
if __name__ == "__main__": print(json.dumps(NexoManager().status(), indent=2))
